...
class WanVideoPipeline(BasePipeline):
    ...
    def fetch_models(self, model_manager: ModelManager):
        ...
        self.audio_patch_embedding = nn.Sequential(nn.Conv3d(50, 2560, kernel_size=(2,1,1), stride=(2,1,1)),
                                nn.Conv3d(2560, 5120, kernel_size=(2,1,1), stride=(2,1,1),padding=(1,0,0)))
        randomref_dim = 20
        concat_dim = 4
        self.masks_embedding = nn.Sequential(
        ...
        self.first_mask_embedding = nn.Sequential(
        ...
        for key in model_manager.state_dict_new_module:
            if "audio_patch_embedding" in key:
                state_dict_new[key.split("audio_patch_embedding.")[1]] = model_manager.state_dict_new_module[key]
        self.audio_patch_embedding.load_state_dict(state_dict_new,strict=True)

        state_dict_new = {}
        for key in model_manager.state_dict_new_module:
            if "masks_embedding" in key:
                state_dict_new[key.split("masks_embedding.")[1]] = model_manager.state_dict_new_module[key]
        self.masks_embedding.load_state_dict(state_dict_new,strict=True)

        state_dict_new = {}
        for key in model_manager.state_dict_new_module:
            if "first_mask_embedding" in key:
                state_dict_new[key.split("first_mask_embedding.")[1]] = model_manager.state_dict_new_module[key]
        self.first_mask_embedding.load_state_dict(state_dict_new,strict=True)
    ...
    def tensor2video(self, frames):
        ...
        frames = ((frames.float() + 1) * (255/2)).clip(0, 255).cpu().numpy().astype(np.uint8)
    ...
    def encode_video(self, input_video, tiled=True, tile_size=(34, 34), tile_stride=(18, 16)):
        # print("encode_video self.device:",self.device,tile_size,tile_stride)
        latents = self.vae.encode(input_video, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        return latents
    
    
    def decode_video(self, latents, tiled=True, tile_size=(34, 34), tile_stride=(18, 16)):
        print("decode_video self.device:",self.device,tile_size,tile_stride)
    ...
    def __call__(
        ...
        self.audio_patch_embedding.to(self.device).to(torch.bfloat16)
        self.masks_embedding.to(self.device).to(torch.bfloat16)
        self.first_mask_embedding.to(self.device).to(torch.bfloat16)

        audio_feature = self.audio_patch_embedding(audio_feature).to(self.device).to(torch.bfloat16) # [1, 20, 104, 60]
        masks_embedding = self.masks_embedding(masks_embedding).to(self.device).to(torch.bfloat16) # [1, 20, 104, 60]
        first_mask_embedding = self.first_mask_embedding(first_mask_embedding).to(self.device).to(torch.bfloat16) # [1, 20, 104, 60]
        image_emb["y"] = image_emb["y"]+first_mask_embedding.unsqueeze(2)
        # Denoise
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            noise_pred_posi = model_fn_wan_video(
                ...
                **prompt_emb_posi, **image_emb,audio_feature=audio_feature,masks=masks_embedding, **extra_input,
            ...
            if cfg_scale != 1.0:
                noise_pred_nega = model_fn_wan_video(
                    self.dit, motion_controller=self.motion_controller, vace=self.vace,
                    x=latents, timestep=timestep,
                    **prompt_emb_nega, **image_emb,audio_feature=audio_feature, masks=masks_embedding,**extra_input,
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # print("text_embedding:",context.shape)
    
    if dit.has_image_input:
        x = torch.cat([x, y], dim=1)  # (b, c_x + c_y, f, h, w)
        clip_embdding = dit.img_emb(clip_feature)
        context = torch.cat([clip_embdding,context], dim=1)
    audio_feature = rearrange(audio_feature, 'b c f h w -> b (f h w) c').contiguous()
    masks = rearrange(masks, 'b c f h w -> b (f h w) c').contiguous()
    x, (f, h, w) = dit.patchify(x)
    x = x+audio_feature+masks
    # print("x:",x.shape)
    freqs = torch.cat([
