...
class WanVideoPipeline_pre(BasePipeline):
    ...
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None, use_usp=False):
        ...
        pipe = WanVideoPipeline_pre(device=device, torch_dtype=torch_dtype)
    ...
    def encode_image(self, times,image, end_image, num_frames, height, width, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
        image = self.preprocess_image(image.resize((width, height))).to(self.device)
        if times==0:
           pre_fix_frames_num=1
        else:
            pre_fix_frames_num=4
        if end_image is not None:
            end_image = self.preprocess_image(end_image.resize((width, height))).to(self.device)
            vae_input = torch.concat([image.transpose(0,1), torch.zeros(3, num_frames-2, height, width).to(image.device), end_image.transpose(0,1)],dim=1)
            msk[:, -1:] = 1
        else:
            vae_input = image.transpose(0, 1)
        ...
        msk = torch.ones_like(y.repeat(1, 1, 21, 1, 1)[:,:1])
        y = y.repeat(1, 1, 21, 1, 1)
        msk[:, :, pre_fix_frames_num:] = 0
        y = torch.cat([y, msk], dim=1)
        y = torch.concat([msk, y])
        y = y.unsqueeze(0)
        y = y.to(dtype=self.torch_dtype, device=self.device)
        return {"clip_feature": None, "y": y}
    ...
    def __call__(
        ...
        #     self.load_models_to_device(["vae"])
        #     image_emb = self.encode_image(times,input_image, end_image, num_frames, height, width, **tiler_kwargs)
